...
const EssayChecker: React.FC<EssayCheckerProps> = ({ onAnalysisComplete }) => {
  ...
  const downloadResult = () => {
    ...
    const content = `CSS EXAM ESSAY ANALYSIS REPORT
===============================================

${analysisResult.isOutlineOnly ? 'OUTLINE-ONLY SUBMISSION' : 'FULL ESSAY EVALUATION'}

${analysisResult.isOutlineOnly ? 
  `Outline Score: ${analysisResult.totalMarks || analysisResult.score}/10` :
  `Total Score: ${analysisResult.totalMarks || analysisResult.score}/100`
}

${analysisResult.isOutlineOnly ? 
  '⚠️ WARNING: Outline-only submission detected. In FPSC exam, this is considered a failure.' :
  ''
}
...
1. Thesis Statement & Topic Understanding: ${analysisResult.evaluation.thesisStatement.score}/10
   Comment: ${analysisResult.evaluation.thesisStatement.comment}

2. Outline: Clarity, Logic, and Direction: ${analysisResult.evaluation.outline.score}/10
   Comment: ${analysisResult.evaluation.outline.comment}

3. Structure & Coherence: ${analysisResult.evaluation.structure.score}/15
   Comment: ${analysisResult.evaluation.structure.comment}

4. Content Depth, Balance & Relevance: ${analysisResult.evaluation.content.score}/20
   Comment: ${analysisResult.evaluation.content.comment}

5. Language Proficiency & Expression: ${analysisResult.evaluation.language.score}/15
   Comment: ${analysisResult.evaluation.language.comment}

6. Critical Thinking & Analytical Reasoning: ${analysisResult.evaluation.criticalThinking.score}/5
   Comment: ${analysisResult.evaluation.criticalThinking.comment}

7. Conclusion: ${analysisResult.evaluation.conclusion.score}/10
   Comment: ${analysisResult.evaluation.conclusion.comment}

8. Word Count & Length Control: ${analysisResult.evaluation.wordCount.score}/15
   Comment: ${analysisResult.evaluation.wordCount.comment}

EXAMINER'S REMARKS:
===================

Strengths:
${analysisResult.examinerRemarks?.strengths.map(strength => `• ${strength}`).join('\n') || '• Not specified'}

Areas for Improvement:
${analysisResult.examinerRemarks?.weaknesses.map(weakness => `• ${weakness}`).join('\n') || '• Not specified'}

Suggestions for Improvement:
${analysisResult.examinerRemarks?.suggestions.map(suggestion => `• ${suggestion}`).join('\n') || '• Not specified'}
...
==============================================`;
    ...
    a.href = url;
    a.download = `essay-analysis-${analysisResult.isOutlineOnly ? 'outline' : 'full'}-${new Date().toISOString().split('T')[0]}.txt`;
    a.click();
          ...
          <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-6 mb-8">
            <div className="text-center">
              <div className="inline-flex items-center justify-center w-24 h-24 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full mb-4">
                <span className="text-3xl font-bold text-white">{analysisResult.totalMarks || analysisResult.score}</span>
              </div>
              <h4 className="text-xl font-semibold text-gray-900 mb-2">
                {analysisResult.isOutlineOnly ? 'Outline Score' : 'Total Score'}
              </h4>
              <p className="text-gray-600">
                {analysisResult.isOutlineOnly ? 'out of 10 points (Outline Only)' : 'out of 100 points'}
              </p>
              {analysisResult.isOutlineOnly && (
                <div className="mt-3 p-3 bg-yellow-100 border border-yellow-300 rounded-lg">
                  <p className="text-yellow-800 text-sm font-medium">
                    ⚠️ Outline-only submission detected. In FPSC exam, this is considered a failure.
                  </p>
                </div>
              )}
            </div>
